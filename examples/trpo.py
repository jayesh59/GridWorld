...
from library.trpoalgo import TRPO
...
parser = argparse.ArgumentParser(description="Training for different setting with TRPO")
parser.add_argument("--random_state", type=int, default=random_state, help="Required random_state of env")
parser.add_argument("--exp_name", type=str, default="TRPO", help="Required Experiment name to run")
parser.add_argument(
    "--init_from_exp", type=str, default=None, help="Optional Experiment name to initilize weights from"
)
parser.add_argument(
    "--log_loc", type=str, default="./logs/trpo/", help="Require log_location for tensorboard"
)
...
if args.test and args.init_from_exp == None:
    raise Exception("Must provide --init_from_exp tag to restore model")
...
if __name__ == "__main__":
    ...
    actor_critic_net = ActorCritic(
        input_size=test_env.observation_space.shape[0],
        action_size=test_env.action_space.n,
        trainable=not args.test,
        exp_name=args.exp_name,
        init_from_exp=args.init_from_exp,
    ...
    if not args.test:
        trpo.learn(train_for_step=train_for_step, test_at_iter=test_at_iter, num_of_test=num_of_test)
    else:
        stat = trpo._test(num_of_test, args.render)
