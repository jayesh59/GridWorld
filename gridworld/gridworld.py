    ...
    def __init__(
        ...
        if self.isDRL:
            self.observation_space = Box(
                low=-2, high=2, shape=((2 * viewsize + 1) * (2 * viewsize + 1),), dtype="int8"
            )
        else:
        ...
        while not done:
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    if self.isDRL:
                        self.formatState(response["state"])
...
class ractGridWorld(gym.Env):
    def __init__(
        ...
        self.observation_space = Box(
            ...
            shape=(self.env.observation_space.shape[0] * repeat_act,),
            dtype="int8",
        )
