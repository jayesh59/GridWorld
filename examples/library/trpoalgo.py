...
class TRPO:
    def __init__(
        ...
        self.mem = Memory(
            self.num_envs, self.n_steps, self.epochs, self.steps_per_epoch, self.shuffle_buffer_size
        )
    ...
    def value_loss(self, states, old_values, returns):
        ...
        if self.cliprange_vf == None:
            ...
            vpred_clipped = old_values + tf.clip_by_value(
                vpred - old_values, -self.cliprange_vf, self.cliprange_vf
            )
    ...
    def surrogate_loss(self, ac_model, states, actions, old_log_probs, adv):
        ...
        return self.policy_cost(new_log_probs, old_log_probs, adv) + self.ent_coef * self.entropy_loss(
            entropy
        )
    ...
    def D_kl(self, ac_model, states, actions, old_log_probs):
        ...
        return tf.reduce_mean(
            tf.reduce_sum(old_action_prob * tf.math.log(old_action_prob / new_action_prob), axis=1)
        )
    ...
    def hessian_vector_product(self, p, params):
        ...
        fisher_vector_product = flatgrad(
            hvp_fn, (params,), self.ac_network.logits.trainable_variables
        ).numpy()
    ...
    def _train(self, states, actions, old_log_probs, adv, old_values, returns):
        ...
        policy_gradient = flatgrad(
            self.surrogate_loss,
            (self.ac_network, states, actions, old_log_probs, adv),
            self.ac_network.logits.trainable_variables,
        ).numpy()
        step_direction = conjugate_grad(
            self.hessian_vector_product,
            policy_gradient,
            self.cg_iters,
            self.residual_tol,
            params=(self.ac_network, states, actions, old_log_probs),
        )
        shs = 0.5 * step_direction.dot(
            self.hessian_vector_product(step_direction, (self.ac_network, states, actions, old_log_probs)).T
        )
        ...
        if np.isnan(theta).any() and self.log:
            print("NaN detected. Skipping update...")
        else:
            self.ac_network.assign_theta(theta)
    ...
    def learn(self, train_for_step=10000, test_at_iter=2, num_of_test=10):
        ...
        for iteration in range(iterations):
            ...
            if iteration % test_at_iter == 0:
                ...
                print(
                    "Global step {} On iteration {} reward {}".format(
                        global_step, iteration, current_avg_reward
                    )
                )
